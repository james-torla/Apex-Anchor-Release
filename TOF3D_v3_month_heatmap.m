...
Version changes:
    %{
    1)Attemped calculation of true anomaly based on orbital elements
    2)Attempted lambert's calculation for Time of Flights 60 days to 180
    days
    3) Need to add logic breaks for when each day obtains an intercepts at
    minimum dV (possibly for each time)
    %}
...
Notes: 
    ...
    2) Release angles to reach Mars must have some prograde velocity
...
Uses: Use this script and relevant data files to analyze trajectories and
    answer relevant research questions including:
        1) How often can we release? Dr. Swan estimates once a day. This is
           the case for some years depending on user constraints for mars distance at intercept
           (3*rSOI, 10*, etc); however, some years have no intercepts.
        2) What is the minimum TOF for possible and actual intercepts (NPC
...
% save Month_Heatmap_v3.mat intdata
toc                          % Use plotintercept for any row in intdata
%% Graphing
% 
% figure;
% Delta_V = intdata{:,5};
% Time_of_Flight = intdata{:,4};
% Time_of_Flight = round(Time_of_Flight);
